...
class MenuDropElement extends HTMLElement {
  ...
  connectedCallback(){
    ...
    this.shadowRoot.addEventListener("keydown",event => {
      ...
      if (event.key == "ArrowRight"){
        ...
        if (event.target.matches(".sub-list > .option")){
          var subList = event.target.closest<SubList>(".sub-list")!, option = this.getOptions(subList.querySelector<List>(".list")!)[0];
          this.open(subList);
          option.focus();
        }
      }
      if (event.key == "ArrowLeft"){
        ...
        if (event.target.matches(".sub-list > .list .option")){
          var subList = event.target.closest<List>(".list")!.closest<SubList>(".sub-list")!, option = subList.querySelector<Option>(".option")!;
          this.close(subList);
          option.focus();
        }
      }
    ...
    this.shadowRoot.addEventListener("pointermove",event => {
      ...
      if (event.pointerType == "mouse"){
        ...
        if (event.target.matches(".sub-list > .option")) this.open(event.target.closest<SubList>(".sub-list")!);
    ...
    window.requestAnimationFrame(() => {
      ...
      if (this.matches("[data-select]") && !this.matches("[data-select='no-appearance']")){
        ...
        if (this.main.querySelector(".option[data-selected]")) this.opener.textContent = this.getTextNodes(this.main.querySelector<Option>(".option[data-selected]")!)[0].textContent;
      }
    });
  }
  open(section: Section = this){
    ...
    Array.from(((section == this) ? this.main : section.closest<List>(".list")!).querySelectorAll<SubList>(".sub-list[data-open]")!).filter(subList => subList != list.closest(".sub-list")).forEach(subList => this.close(subList,false));
  ...
  select(option: number | string | Option){
    ...
    if (typeof option == "string") option = this.main.querySelector<Option>(`[data-value="${option}"]`)!;
    if (!this.main.contains(option)) return;
    this.getOptions(option.closest<List>(".list")!).filter(option => option.matches("[data-selected]")).forEach(option => option.removeAttribute("data-selected"));
